...
Engine type and fuel system fields are specified in the addendum [AIRCRAFTS_ENGINE_FUEL_EXTENSION.md](../physics/AIRCRAFTS_ENGINE_FUEL_EXTENSION.md).
...
CREATE TABLE IF NOT EXISTS aircrafts (
    ...
    -- Engine & Propeller
    engine_type               ENUM('piston','turboprop','turbojet','turbofan','electric') NOT NULL DEFAULT 'turbofan' COMMENT 'Discriminator for engine-class physics. Returned as numeric enum index.',
    engine_count              TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Number of engines installed. >=1.',
    prop_diameter_m           DECIMAL(4,2) NULL COMMENT 'Propeller disc diameter in meters. NULL for jet/electric.',
    prop_rotation_dir         ENUM('cw','ccw') NULL COMMENT 'Prop rotation viewed from the cockpit. NULL for jet/electric.',
    prop_inertia_kgm2         DECIMAL(8,3) NULL COMMENT 'Moment of inertia of the spinning rotor assembly. NULL for jet/electric.',
    prop_rpm_max              SMALLINT UNSIGNED NULL COMMENT 'Max steady-state propeller RPM. NULL for jet/electric.',

    -- Fuel
    fuel_capacity_kg          DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT 'Usable fuel mass at 100% tanks, in kilograms.',
    fuel_burn_rate_kg_per_s_max  DECIMAL(8,5) NOT NULL DEFAULT 0.00000 COMMENT 'Fuel mass-flow at 100% throttle, ISA sea level, total across all engines.',
    fuel_burn_rate_kg_per_s_idle DECIMAL(8,5) NOT NULL DEFAULT 0.00000 COMMENT 'Fuel mass-flow at idle, total across all engines.',
...
**Engine & Fuel enum encoding:**

`engine_type` index map: `0=piston, 1=turboprop, 2=turbojet, 3=turbofan, 4=electric`.

`prop_rotation_dir` index map: `0=cw, 1=ccw`. `NULL` must be emitted as JSON `null` (not `0`).

**Validation rules (for future create/update endpoints):**
- If `engine_type IN ('piston','turboprop')`: `prop_diameter_m`, `prop_rotation_dir`, `prop_inertia_kgm2`, `prop_rpm_max` MUST be NOT NULL.
- If `engine_type IN ('turbojet','turbofan','electric')`: the same four `prop_*` columns MUST be NULL.
- If `engine_type = 'electric'`: `fuel_capacity_kg`, `fuel_burn_rate_kg_per_s_max`, `fuel_burn_rate_kg_per_s_idle` MUST all be `0`.
...
INSERT INTO aircrafts (
    ...
    engine_type, engine_count, prop_diameter_m, prop_rotation_dir, prop_inertia_kgm2, prop_rpm_max,
    fuel_capacity_kg, fuel_burn_rate_kg_per_s_max, fuel_burn_rate_kg_per_s_idle,
    model_file, model_target_size, model_rotation_y, thumbnail_url,
    ...
    'turbofan', 4, NULL, NULL, NULL, NULL,
    23000.00, 2.15000, 0.18000,
...
- `engine_type` must be returned as its enum numeric index (0=piston, 1=turboprop, 2=turbojet, 3=turbofan, 4=electric). `prop_rotation_dir` must be returned as its enum numeric index (0=cw, 1=ccw) or `null`. The four `prop_*` columns are `null` for jet/electric engines.
...
- The nested `aircraft` object includes the same engine/fuel fields as `GET /api/aircrafts/:id`
...
| `engine_type` | Discriminator that gates prop-only physics (P-factor, propwash, gyro precession). Jets/turbofans skip those passes. |
| `engine_count` | Multiplier for thrust noise/audio and (future) engine-out scenarios. |
| `prop_diameter_m` | Propeller disc diameter — input to the propeller torque model. |
| `prop_rotation_dir` | P-factor sign and gyroscopic precession direction. |
| `prop_inertia_kgm2` | Moment of inertia of the spinning rotor assembly — reaction torque and gyroscopic precession magnitude. |
| `prop_rpm_max` | Max steady-state propeller RPM — ceiling for propeller torque calculations. |
